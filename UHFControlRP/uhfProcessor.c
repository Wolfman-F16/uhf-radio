	...
	if (uhfKeyState.FUSEL == FS_MNL) {
		...
		case MLOAD: {
			...
			case 1:
				...
				break;
			case 14:
				memcpy(&(uhfMWOD[currMWodIndex].ch_14[0]), &(radio.frequency[1]), MAX_DOM);
				playTone(BEEP);
				currMWodIndex++;
				...
				break;
			case 15:
				memcpy(&(uhfMWOD[currMWodIndex].ch_15[0]), &(radio.frequency[0]), FREQ_ARRAY_SIZE);
				playTone(BEEP);
				break;
			case 16:
				memcpy(&(uhfMWOD[currMWodIndex].ch_16[0]), &(radio.frequency[0]), FREQ_ARRAY_SIZE);
				playTone(BEEP);
				break;
			case 17:
				memcpy(&(uhfMWOD[currMWodIndex].ch_17[0]), &(radio.frequency[0]), FREQ_ARRAY_SIZE);
				playTone(BEEP);
				break;
			case 18:
				memcpy(&(uhfMWOD[currMWodIndex].ch_18[0]), &(radio.frequency[0]), FREQ_ARRAY_SIZE);
				playTone(BEEP);
				break;
			case 19:
				memcpy(&(uhfMWOD[currMWodIndex].ch_19[0]), &(radio.frequency[0]), FREQ_ARRAY_SIZE);
				playTone(BEEP);
				break;
			case 20:
				memcpy(&(uhfMWOD[currMWodIndex].ch_20[0]), &(radio.frequency[0]), FREQ_ARRAY_SIZE);
				playTone(BEEP);
				break;
			default:
				break;
		...
		case FMTCHG:
			...
			// check for ch 20 - 5
			if (radio.channel > 4) {
				memcpy(uhfFMT.ch[MAX_CHANNELS - radio.channel], &radio.frequency, FREQ_ARRAY_SIZE);
				eeprom_write_block(eeFmtNets[MAX_CHANNELS - radio.channel], &radio.frequency, FREQ_ARRAY_SIZE);
...
void setLoad() {
	if (memcmp_P(&(radio.frequency[0]), veropFreq, 4) == 0) {
		...
		// but channel 20 holds training/common net!? Apache SWOD loading states, you program ch 20 with training net freq 300.0xx
	}
}

/**
 * op mode is shown with ch 20 selected. if preset is set and radio is in normal mode (verop)
 * then status loads frequency for selected channel
 */
void setStatus() {
	if (uhfKeyState.FUSEL == FS_PRESET && uhfOpMode == VEROP) {
		if (radio.frequency[0] == 'A') {
			if (radio.channel > 4) {
				memcpy(&(radio.frequency[0]), &(uhfFMT.ch[MAX_CHANNELS - radio.channel][0]), FREQ_ARRAY_SIZE);
			}
		} else {
			eeprom_read_block(
					&(radio.frequency[0]),
...
/*
 * sends data to SPI interface. if debug is enabled, it also sends data to USART.
 */
...
void checkDom() {
	...
	if (uhfOpMode == VEROP && uhfKeyState.FUSEL == FS_MNL && uhfControl.ch20 == TRUE) {
		for (i = 0; i < MAX_MWOD; i++) {
			if (radio.frequency[1] == uhfMWOD[i].ch_14[0] && radio.frequency[2] == uhfMWOD[i].ch_14[1]) {
				playTone(BEEP);
				break;
			}
			DBG_WARN("\r\nChecked ",10);
...
void goActive() {
	...
	eeprom_read_block(&(commonNet[0]), &(eePresetFrequencies[MAX_CHANNELS - 1][0]), FREQ_ARRAY_SIZE);
	...
	if (match) {
		...
		if (radio.frequency[4] == '0') {
			...
			if (netNumber < 5) {
				// selected net is one of presetChannel 19 to 15 (SWOD). There is always a frequency for SWOD, but maybe not valid
			...
			if (netNumber < 16) {
				// active, no audio (only permissible, because FMT-NETs are loaded non-volatile in eeprom)
				return;
			}
		}
