...
void processInputData(uint8_t keyStatus) {
	...
	case COMMON_WIRE_05:
		...
		// test depressed, doesn't work, because test flag is deleted by reading value
	...
	case COMMON_WIRE_11:
		...
		case (1 << KEY7):
			if (uhfKeyState.OFF == MS_OFF) {
				if(uhfKeyState.MODE != MS_ADF) {	/* too many calls of setUhfOn() so key stroke is */
					sendUsbMessage(KEY_MAIN);				/* separate here instead of within setUhfOn() */
					sendUsbMessage(NOT_USED);				/* only one key stroke should be sent, so release here */
				}
				setUhfOn();
		...
		// UHF Mode (sort of) depressed, doesn't work, because UHF mode flag is deleted by reading value
	...
	case COMMON_WIRE_12:
		...
		// squelch depressed, doesn't work, because squelch flag is deleted by reading value
...
void processChannel() {
	...
	if (keyStatus == MASK_CH_15) {
		...
		if (uhfOpMode == VEROP && uhfKeyState.FUSEL == FS_PRESET && uhfControl.invalidSWOD == 1) {
	...
	if (keyStatus == MASK_CH_16) {
		...
		sendKeyStroke(15);
		radio.channel = 16;
		if (uhfOpMode == VEROP && uhfKeyState.FUSEL == FS_PRESET && uhfControl.invalidSWOD == 2) {
			playTone(BEEP);
	...
	if (keyStatus == MASK_CH_17) {
		...
		sendKeyStroke(16);
		radio.channel = 17;
		if (uhfOpMode == VEROP && uhfKeyState.FUSEL == FS_PRESET && uhfControl.invalidSWOD == 3) {
			playTone(BEEP);
	...
	if (keyStatus == MASK_CH_18) {
		...
		sendKeyStroke(17);
		radio.channel = 18;
		if (uhfOpMode == VEROP && uhfKeyState.FUSEL == FS_PRESET && uhfControl.invalidSWOD == 4) {
			playTone(BEEP);
	...
	if (keyStatus == MASK_CH_19) {
		...
		checkDom();
		if (uhfOpMode == VEROP && uhfKeyState.FUSEL == FS_PRESET && uhfControl.invalidSWOD == 5) {
			playTone(BEEP);
...
void forcedReadFreq() {
	...
	for (currentKeyRow = COMMON_WIRE_00; currentKeyRow < COMMON_WIRE_10; currentKeyRow++) {
...
void forcedReadUhfState() {
	...
	for (currentKeyRow = COMMON_WIRE_13; currentKeyRow > COMMON_WIRE_02; currentKeyRow--) {
		MULTIPLEXPORT &= 0xF0; // erase lower nibble (key rows)
		MULTIPLEXPORT |= currentKeyRow; // only works because pins 0 to 3 are used
		temp = 128;
		DBG_DELAY_8BIT(temp);
